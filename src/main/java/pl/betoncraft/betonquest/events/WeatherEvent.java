...
public class WeatherEvent extends QuestEvent {
	...
	public static TypeMetadata getMetadata() {
		return new DefaultTypeMetadata()
		        .setName(new StringText("Weather"))
		        .setDescription(new StringText("Sets the weather on the player's world"))
		        .setFormat(Format.argument().require(
		                new Argument()
		                        .setName(new StringText("Weather type"))
		                        .setDescription(new StringText("The name of the weather type to set"))
		                        .setData(Data.variant("weather",
		                                Arrays.asList("sun", "clear", "rain", "rainy", "storm", "thunder")))));
	}
